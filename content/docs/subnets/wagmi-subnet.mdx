...
Detailed description of how to do this can be found in [Customize a Subnet](/docs/subnets/manage-subnets/upgrade/customize-subnet#network-upgrades-enabledisable-precompiles) tutorial. Here's a summary:
...
3. These upgrades must be specified in a file named `upgrade.json` placed in the same directory where [`config.json`](/docs/subnets/manage-subnets/upgrade/customize-subnet#avalanchego-chain-configs) resides: `{chain-config-dir}/{blockchainID}/upgrade.json`.
...
Detailed explanations of feeManagerConfig can be found in [here](/docs/subnets/manage-subnets/upgrade/customize-subnet#configuring-dynamic-fees), and for the contractNativeMinterConfig in [here](/docs/subnets/manage-subnets/upgrade/customize-subnet#minting-native-coins).
...
We will use [Remix](https://remix.ethereum.org/) online Solidity IDE and the [Core Browser Extension](https://support.avax.network/en/articles/6066879-core-extension-how-do-i-add-the-core-extension). Core comes with WAGMI network built-in. MetaMask will do as well but you will need to [add WAGMI](/docs/subnets/wagmi-case-study#adding-wagmi-to-core) yourself.
