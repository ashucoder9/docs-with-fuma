...
- Your wallet has a balance of the Subnet Native Token(Specified under _alloc_ in your [Genesis File](/docs/subnets/manage-subnets/upgrade/customize-subnet#genesis)).
...
You can use Subnet-EVM just like you use C-Chain and EVM tools. Only differences are `chainID` and RPC URL. For example you can deploy your contracts with [hardhat quick start guide](/docs/dapps/smart-contract-dev/toolchains/hardhat) by changing `url` and `chainId` in the `hardhat.config.ts`.