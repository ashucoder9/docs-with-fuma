...
[Install Avalanche CLI](/docs/tooling/guides/install-avalanche-cli)
...
The `primary addValidator` command adds an Avalanche node as a validator in the Avalanche Primary Network with [AddPermissionlessValidatorTx](/docs/api-reference/standards/guides/banff-changes#addpermissionlessvalidatortx).

This command requires the node's BLS key and proof of possession key, more information regarding BLS can be found [here](/docs/api-reference/p-chain/transaction-format#proof-of-possession).

To get a node's BLS key and proof of possession key, call info.getNodeID API as shown [here](/docs/api-reference/info-api#infogetnodeid)
...
The `subnet changeOwner` changes the owner of the deployed Subnet. This command currently only works on Subnets deployed to Devnet, Fuji or Mainnet.
...
The `subnet export` command write the details of an existing Subnet deploy to a file. The command prompts for an output path. You can also provide one with the `--output` flag.
...
After you update your validator's configuration, you need to restart your validator manually. If you provide the `--avalanchego-chain-config-dir` flag, this command attempts to write the upgrade file at that path. Refer to [this doc](/docs/nodes/configure/configs-flags#subnet-chain-configs) for related documentation.
...
The `node sync` command enables all nodes in a cluster to be bootstrapped to a Subnet. You can check the Subnet bootstrap status by calling avalanche `node status <clusterName> --subnet <subnetName>`
...
The `node update subnet` command updates all nodes in a cluster with latest Subnet configuration and You can check the updated Subnet bootstrap status by calling avalanche `node status <clusterName> --subnet <subnetName>`
