...
The document [Upgrade a Subnet](/docs/subnets/manage-subnets/upgrade/considerations) describes all the background information required regarding Subnet upgrades.
...
This tutorial assumes that you have already [installed](/docs/tooling/guides/install-avalanche-cli) Avalanche-CLI. It assumes you have already created and deployed a Subnet called `testSubnet`.

Generate the Upgrade File[​](#generate-the-upgrade-file "Direct link to heading")
---------------------------------------------------------------------------------

The [Precompiles](/docs/subnets/manage-subnets/upgrade/customize-subnet#network-upgrades-enabledisable-precompiles) documentation describes what files the network upgrade requires, and where to place them.
...
Refer to [Precompiles](/docs/subnets/manage-subnets/upgrade/customize-subnet#precompiles) for a description of available precompiles and how to configure them.

Make sure you understand precompiles thoroughly and how to configure them before attempting to continue.

For every precompile in the list, the wizard guides you to provide correct information by prompting relevant questions. For the sake of this tutorial, select `Transaction Allow List`. The document [Restricting Who Can Submit Transactions](/docs/subnets/manage-subnets/upgrade/customize-subnet#restricting-who-can-submit-transactions) describes what this precompile is about.
