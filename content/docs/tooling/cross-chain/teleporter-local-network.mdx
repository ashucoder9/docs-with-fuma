...
- [Cross Chain References](/docs/cross-chain/quicklinks)
...
- [Avalanche-CLI installed](/docs/tooling/guides/install-avalanche-cli)

Create Subnet Configurations[​](#create-subnet-configurations "Direct link to heading")
---------------------------------------------------------------------------------------

Let's create a Subnet called `<subnet1>` with the latest Subnet-EVM version, a chain ID of 1, TOKEN1 as the token name, and with default Subnet-EVM parameters (more information regarding Subnet creation can be found [here](/docs/subnets/build-first-subnet#create-your-subnet-configuration)):
...
- [AWM Teleporter Relayer](https://github.com/ava-labs/awm-relayer) is installed, configured and executed in background (A Relayer [listens](/docs/cross-chain/teleporter/overview#data-flow) for new messages being generated on a source Subnet and sends them to the destination Subnet.)
