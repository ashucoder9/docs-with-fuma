---
title: Deploy Teleporter on a Local Network
description: This how-to guide focuses on deploying Teleporter-enabled Subnets to a local Avalanche network.
---
...
Let's create a Subnet called `<subnet1>` with the latest Subnet-EVM version, a chain ID of 1, TOKEN1 as the token name, and with default Subnet-EVM parameters (more information regarding Subnet creation can be found [here](/build/subnets/build-first-subnet#create-your-subnet-configuration)):
...
- [AWM Teleporter Relayer](https://github.com/ava-labs/awm-relayer) is installed, configured and executed in background (A Relayer [listens](/build/cross-chain/teleporter/overview#data-flow) for new messages being generated on a source Subnet and sends them to the destination Subnet.)
