---
title: Setup a Devnet Using Avalanche-CLI
...
*   [Created a Subnet configuration `<SubnetName>`](/build/subnets/deploy-a-subnet/fuji-testnet#create-an-evm-subnet) based on SubnetEVM.

Note: the tutorial is based in AWS, but Devnets can also be created and operated in other supported cloud providers, such as GCP.

Choose a Deployment Method[​](#choose-a-deployment-method "Direct link to heading")
-----------------------------------------------------------------------------------

You can either use a sequence of prompt-based commands to get CLI to ask you all the required information for the setup, or execute one command than encompasses all steps, with appropriate command line flags for all the needed info.
...
*   creating a devnet, create a Subnet based on a Custom VM, and deploy it (similar to [this one](/tooling/create-avalanche-nodes/deploy-custom-vm))
*   creating a devnet with warp-enabled Subnets

Step by Step[​](#step-by-step "Direct link to heading")
-------------------------------------------------------

### Creating the Devnet[​](#creating-the-devnet "Direct link to heading")
...
After that, you will be asked which AvalancheGo version you want to install in the nodes. Select the one associated to `<subnetName>`.
...
### Deploying the Subnet into the Devnet[​](#deploying-the-subnet-into-the-devnet "Direct link to heading")
...
So, next step is to deploy `<subnetName>` into `<clusterName>`. So as to create the Subnet and Blockchain transactions needed for posterior syncing and validation of the Subnet.

While the `subnet deploy` command can be used [as described here](/build/subnets/deploy-a-subnet/fuji-testnet#deploy-the-subnet), deploying into Devnets is a subset of that operation, and so a quickest command is provided.
...
### Syncing the Subnet[​](#syncing-the-subnet "Direct link to heading")
...
### Validating the Subnet[​](#validating-the-subnet "Direct link to heading")

Latest step is to add the nodes as Subnet validators, so as the Devnet can change the Subnet state under user request.

While you can follow [the instructions here](/tooling/create-avalanche-nodes/validate-subnet#be-a-subnet-validator), for this tutorial we opt for providing the flag `--default-validator-params`, that set for the user most common Validator params:
...
Single Command[​](#single-command "Direct link to heading")
-----------------------------------------------------------

### Create a Devnet and Deploy a Subnet into It[​](#create-a-devnet-and-deploy-a-subnet-into-it "Direct link to heading")

CLI provides the `devnet wiz` command that takes care of all the mentioned steps.

As other CLI commands, all the options can be specified by command line flag instead of having the user interactively provide the inputs.
...
### Create a Devnet, Create a Custom VM based Subnet, and Deploy It[​](#create-a-devnet-create-a-custom-vm-based-subnet-and-deploy-it "Direct link to heading")

#### Prerequisites[​](#prerequisites-1 "Direct link to heading")

You will need to have:

*   Created a Custom VM, as described [here](/build/virtual-machines/introduction-vms).

#### Custom VM[​](#custom-vm "Direct link to heading")

The tutorial will create exactly the same Custom VM as [here](/tooling/create-avalanche-nodes/deploy-custom-vm).

Based on the Custom VM tutorial, we will be deploying the [TokenVM](https://github.com/ava-labs/hypersdk/tree/main/examples/tokenvm) example built with the HyperSDK.
...
*   `<subnetConfigPath>` [Subnet Config](/tooling/create-avalanche-nodes/deploy-custom-vm#subnet-config)
*   `<avagoConfigPath>` [AvalancheGo Config](/tooling/create-avalanche-nodes/deploy-custom-vm#avalanchego-flags)

#### Execute Wiz[​](#execute-wiz "Direct link to heading")
