---
title: Deploy a Custom VM Using Avalanche-CLI
sidebarTitle: Deploy a Custom VM
...
*   Created a Cloud Server node as described [here](/tooling/create-avalanche-nodes/run-validators-aws)
*   Created a Custom VM, as described [here](/build/virtual-machines/introduction-vms).
*   Set up a key to be able to pay for the Fuji Fees, as described [here](/build/subnets/deploy-a-subnet/fuji-testnet).

Currently, we only support AWS cloud services, but we plan to add support for more cloud services in the near future.

Deploying the VM[​](#deploying-the-vm "Direct link to heading")
---------------------------------------------------------------

We will be deploying the [TokenVM](https://github.com/ava-labs/hypersdk/tree/main/examples/tokenvm) example built with the HyperSDK.
...
The following contents will serve as the chain genesis. They were generated using `token-cli` as shown [here](https://github.com/ava-labs/hypersdk/blob/main/examples/tokenvm/scripts/run.sh).
...
Create the Subnet[​](#create-the-subnet "Direct link to heading")
-----------------------------------------------------------------
...
Deploy Subnet on Fuji[​](#deploy-subnet-on-fuji "Direct link to heading")
-------------------------------------------------------------------------
...
Save the following content (as defined [here](https://github.com/ava-labs/hypersdk/blob/main/examples/tokenvm/tests/e2e/e2e_test.go)) into a file with path `<avagoFlagsPath>` (for example `~/tokenvm_avago.json`):
...
`token-cli` as shown [here](https://github.com/ava-labs/hypersdk/blob/main/examples/tokenvm/scripts/run.sh). Save the following content (generated by this [script](https://github.com/ava-labs/hypersdk/blob/main/examples/tokenvm/scripts/run.sh)) in a known file path (for example `~/tokenvm_chain.json`):
...
### Subnet Config[​](#subnet-config "Direct link to heading")

Save the following content (generated by this [script](https://github.com/ava-labs/hypersdk/blob/main/examples/tokenvm/scripts/run.sh)) in a known path (for example `~/tokenvm_subnet.json`):
