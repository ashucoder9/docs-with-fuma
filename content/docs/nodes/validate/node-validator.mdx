---
title: Add a Node to the Validator Set
sidebarTitle: Node --> Validator
---
Introduction[​](#introduction "Direct link to heading")
-------------------------------------------------------

The [Primary Network](/learn/primary-network) is inherent to the Avalanche platform and validates Avalanche's built-in blockchains. In this tutorial, we'll add a node to the Primary Network on Avalanche.
...
You've completed [Run an Avalanche Node](/nodes/run-a-node/manually) and are familiar with [Avalanche's architecture](/learn/primary-network). In this tutorial, we use [AvalancheJS](/tooling/avalanche-js) and [Avalanche's Postman collection](/tooling/avalanche-go-postman-collection/setting-up-postman) to help us make API calls.
...
Add a Validator with Core extension[​](#add-a-validator-with-core-extension "Direct link to heading")
-----------------------------------------------------------------------------------------------------

First, we show you how to add your node as a validator by using [Core web](https://core.app/).

### Retrieve the Node ID[​](#retrieve-the-node-id "Direct link to heading")

Get your node's ID by calling [`info.getNodeID`](/api-reference/info-api#infogetnodeid):
...
### Add as a Validator[​](#add-as-a-validator "Direct link to heading")

Connect [Core extension](https://join.core.app/extension) to [Core web](https://core.app/), and go the 'Staking' tab. Here, choose 'Validate' from the menu.

Fill out the staking parameters. They are explained in more detail in [this doc](/nodes/validate/how-to-stake). When you've filled in all the staking parameters and double-checked them, click `Submit Validation`. Make sure the staking period is at least 2 weeks, the delegation fee rate is at least 2%, and you're staking at least 2,000 AVAX on Mainnet (1 AVAX on Fuji Testnet). A full guide about this can be found [here](https://support.avax.network/en/articles/8117267-core-web-how-do-i-validate-in-core-stake).
<iframe
  width="560"
  height="315"
  src="https://www.youtube.com/embed/1M0LZbuHO5Q?si=dURH-8y65Chs_21-"
...
Calling [`platform.getPendingValidators`](/api-reference/p-chain/api#platformgetpendingvalidators) verifies that your transaction was accepted. Note that this API call should be made before your node's validation start time, otherwise, the return will not include your node's id as it is no longer pending.

You can also call [`platform.getCurrentValidators`](/api-reference/p-chain/api#platformgetcurrentvalidators) to check that your node's id is included in the response.

That's it!

Add a Validator with AvalancheJS[​](#add-a-validator-with-avalanchejs "Direct link to heading")
-----------------------------------------------------------------------------------------------

We can also add a node to the validator set using [AvalancheJS](/tooling/avalanche-js).

### Install AvalancheJS[​](#install-avalanchejs "Direct link to heading")
...
`git clone [[email protected]](https://docs.avax.network/cdn-cgi/l/email-protection):ava-labs/avalanchejs.git`
...
### Fuji Workflow[​](#fuji-workflow "Direct link to heading")
...
#### Network Setting[​](#network-setting "Direct link to heading")

The following settings work when using a local node started with [`--network-id=fuji`](/nodes/configure/configs--flags#network-id):
...
However, to connect directly to the [Avalanche Fuji Testnet API server](/tooling/rpc-providers), the following changes are needed:
...
<Accordion title="Details about Bech32 Addresses">
...
#### Settings for Validation[​](#settings-for-validation "Direct link to heading")
...
#### Node ID[​](#node-id "Direct link to heading")

This is the node ID of the validator being added. See [above section](#retrieve-the-node-id) on how to retrieve the node id by using API [`info.getNodeID`](/api-reference/info-api#infogetnodeid).

#### Staking Period[​](#staking-period "Direct link to heading")
...
#### Delegation Fee Rate[​](#delegation-fee-rate "Direct link to heading")
...
#### Stake Amount[​](#stake-amount "Direct link to heading")
...
#### Execute the Code[​](#execute-the-code "Direct link to heading")
...
#### Customizing Addresses[​](#customizing-addresses "Direct link to heading")
...
### Mainnet Workflow[​](#mainnet-workflow "Direct link to heading")

The Fuji workflow above can be adapted to Mainnet with the following modifications:

*   The correct private key.
*   Network setting should be to a Mainnet node, either [a local node on Mainnet](/nodes/configure/configs--flags#network-id) or [Avalanche Mainnet API server](/tooling/rpc-providers#using-the-public-api-nodes) where `api.avax.network` should be used for the `ip`.
