...
If you installed your node using the [installer script](/nodes/run-a-node/using-install-script/installing-avalanche-go), to upgrade your node, just run the installer script again.
...
Building the node [from source](/nodes/maintain/upgrade#build-from-source) is recommended if you're a developer looking to experiment and build on Avalanche.
...
If you have a node, you can subscribe to the [avalanche notify service](/nodes/maintain/enroll-in-avalanche-notify) with your node ID to be notified about new releases.
...
`git clone [[email protected]](https://docs.avax.network/cdn-cgi/l/email-protection):ava-labs/avalanchego.git`
