...
sidebarTitle: Node Configand Maintenance
---
Advanced Node Configuration[​](#advanced-node-configuration "Direct link to heading")
-------------------------------------------------------------------------------------
...
*   `admin` - [Admin API](/api-reference/admin-api) will be enabled
...
*   `index` - [Index API](/api-reference/index-api) will be enabled
...
The file that configures node operation is `~/.avalanchego/configs/node.json`. You can edit it to add or change configuration options. The documentation of configuration options can be found [here](/nodes/configure/configs--flags). Configuration may look like this:
...
The script also creates an empty C-Chain config file, located at `~/.avalanchego/configs/chains/C/config.json`. By editing that file, you can configure the C-Chain, as described in detail [here](/nodes/configure/configs--flags).
...
Now you can [interact with your node](/api-reference/standards/guides/api-calls), [stake your tokens](/nodes/validate/what-is-staking), or level up your installation by setting up [node monitoring](/nodes/maintain/monitoring) to get a better insight into what your node is doing. Also, you might want to use our [Postman Collection](/tooling/avalanche-go-postman-collection/setting-up-postman) to more easily issue commands to your node.

Finally, if you haven't already, it is a good idea to [back up](/nodes/maintain/backup-restore) important files in case you ever need to restore your node to a different machine.
