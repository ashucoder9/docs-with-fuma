...
- To use a third-party service to host your node or run a validator, [see here](/docs/nodes/run-a-node/on-third-party-services/amazon-web-services) for dedicated tutorials.  
- If you're just interested in setting up a node for staking, it's recommended to follow the [AvalancheGo Install Script](/docs/nodes/run-a-node/using-install-script/installing-avalanche-go) tutorial.
</Accordion>
</Accordions>
...
For more information on the requests available at these endpoints, please see the [AvalancheGo API Reference](/docs/api-reference/quicklinks) documentation.
...
To track a Subnet with your node, head to the [Subnet Node](/docs/nodes/run-a-node/subnet-nodes) tutorial.