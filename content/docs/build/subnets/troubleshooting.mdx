---
title: Troubleshooting Subnet Deployments
...
2.  Use Avalanche-CLI to deploy an older version of AvalancheGo by using the `--avalanchego-version` flag. Both the [`subnet deploy`](/tooling/avalanche-cli#subnet-deploy) and [`network start`](/tooling/avalanche-cli#network-start) commands support setting the AvalancheGo version explicitly.
