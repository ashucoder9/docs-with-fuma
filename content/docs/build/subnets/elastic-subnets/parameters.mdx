---
title: Elastic Subnets Parameters
sidebarTitle: Parameters
---
Avalanche Permissioned Subnets can be turned into Elastic Subnets via the [`TransformSubnetTx`](/api-reference/standards/guides/banff-changes#transformsubnettx) transaction. `TransformSubnetTx` specifies a set of structural parameters for the Elastic Subnet. This reference describes these structural parameters and illustrates the constraints they must satisfy.

Elastic Subnet Parameters[​](#elastic-subnet-parameters "Direct link to heading")
---------------------------------------------------------------------------------
...
### `AssetID`[​](#assetid "Direct link to heading")
...
### `InitialSupply`[​](#initialsupply "Direct link to heading")
...
### `MaximumSupply`[​](#maximumsupply "Direct link to heading")
...
### `MaxConsumptionRate`[​](#maxconsumptionrate "Direct link to heading")

`MaxConsumptionRate` has type `uint64` and it's the maximal rate a validator can earn if the `UptimeRequirement` is satisfied. If `StakingPeriod` == `MaxStakeDuration` == `MintingPeriod`, the validator will earn the `MaxConsumptionRate`. You can find more details about it in the [Reward Formula section](#reward-formula). The following constraints apply:
...
### `MaxValidatorWeightFactor`[​](#maxvalidatorweightfactor "Direct link to heading")
...
### `UptimeRequirement`[​](#uptimerequirement "Direct link to heading")
...
Consider an Elastic Subnet validator which stakes a StakeStake amount `AssetID` for StakingPeriodStakingPeriod seconds.

Assume that at the start of the staking period there is a SupplySupply amount of `AssetID` in the Subnet. The maximum amount of Subnet asset is MaximumSupplyMaximumSupply `AssetID`.

Then at the end of its staking period, a responsive Elastic Subnet validator receives a reward calculated as follows:

Reward\=(MaximumSupply−Supply)×StakeSupply×StakingPeriodMintingPeriod×EffectiveConsumptionRateReward = \\left(MaximumSupply - Supply \\right) \\times \\frac{`Stake`}{Supply} \\times \\frac\{Staking Period}\{Minting Period} \\times EffectiveConsumptionRate

where

MaximumSupply−Supply\=the number of tokens left to emit in the subnetMaximumSupply - Supply = \\text\{the number of tokens left to emit in the subnet} StakeSupply\=the individual's stake as a percentage of all available tokens in the network\\frac{`Stake`}{Supply} = \\text\{the individual's stake as a percentage of all available tokens in the network} StakingPeriodMintingPeriod\=time tokens are locked up divided by the MintingPeriod\\frac{StakingPeriod}{MintingPeriod} = \\text\{time tokens are locked up divided by the $MintingPeriod$} MintingPeriod is one year as configured by the Primary Network).\\text\{$MintingPeriod$ is one year as configured by the Primary Network).} EffectiveConsumptionRate\=EffectiveConsumptionRate = MinConsumptionRatePercentDenominator×(1−StakingPeriodMintingPeriod)+MaxConsumptionRatePercentDenominator×StakingPeriodMintingPeriod\\frac{MinConsumptionRate}{PercentDenominator} \\times \\left(1- \\frac\{Staking Period}\{Minting Period}\\right) + \\frac{MaxConsumptionRate}{PercentDenominator} \\times \\frac\{Staking Period}\{Minting Period}
...
MinConsumptionRate≤EffectiveConsumptionRate≤MaxConsumptionRateMinConsumptionRate \\leq EffectiveConsumptionRate \\leq MaxConsumptionRate
...
MaxReward\=(MaximumSupply−Supply)×StakeSupply×MaxConsumptionRatePercentDenominatorMax Reward = \\left(MaximumSupply - Supply \\right) \\times \\frac{`Stake`}{Supply} \\times \\frac{MaxConsumptionRate}{PercentDenominator}
...
Delegators Weight Checks[​](#delegators-weight-checks "Direct link to heading")
-------------------------------------------------------------------------------
...
where MaxValidatorWeightFactorMaxValidatorWeightFactor and MaxValidatorStakeMaxValidatorStake are the Elastic Subnet Parameters described above.
...
Notes on Percentages[​](#notes-on-percentages "Direct link to heading")
-----------------------------------------------------------------------
...
Primary Network Parameters on Mainnet[​](#primary-network-parameters-on-mainnet "Direct link to heading")
---------------------------------------------------------------------------------------------------------
