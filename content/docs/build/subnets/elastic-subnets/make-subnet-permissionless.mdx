---
title: How to Transform a Permissioned Subnet into an Elastic Subnet
sidebarTitle: Make Subnet Permissionless
---
Elastic Subnets are permissionless Subnets. More information can be found [here](/build/subnets/elastic-subnets/parameters).
...
*   [Avalanche-CLI installed](/tooling/guides/install-avalanche-cli)
*   You have deployed a permissioned Subnet on [local](/build/subnets/deploy-a-subnet/local-network), on [Fuji](/build/subnets/deploy-a-subnet/fuji-testnet) or on [Mainnet](/build/subnets/deploy-a-subnet/avalanche-mainnet)
...
Next, select the Elastic Subnet config. You can choose to use the default values detailed [here](/build/subnets/elastic-subnets/parameters#primary-network-parameters-on-mainnet) or customize the Elastic Subnet config. To bypass the prompt, you can use `--default` flag to use the default Elastic Subnet config.
...
Both stored key usage and ledger usage are enabled on Fuji (see more on creating keys [here](/build/subnets/deploy-a-subnet/fuji-testnet#private-key)) while only ledger usage is enabled on Mainnet (see more on setting up your ledger [here](/build/subnets/deploy-a-subnet/avalanche-mainnet#setting-up-your-ledger)).

To transform a permissioned Subnet into Elastic Subnet on public networks, users are required to provide the keys that control the Subnet defined during the Subnet deployment process (more info on keys in Fuji can be found [here](/build/subnets/deploy-a-subnet/fuji-testnet#deploy-the-subnet), while more info on ledger signing in Mainnet can be found [here](/build/subnets/deploy-a-subnet/avalanche-mainnet#deploy-the-subnet)).
