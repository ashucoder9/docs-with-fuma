---
title: Deploy a Smart Contract on Your Subnet-EVM Using Remix and Core
sidebarTitle: Deploy a Smart Contract
---
...
- [A Subnet and EVM blockchain](/build/subnets/deploy-a-subnet/fuji-testnet) has been created
- Your Node is currently validating your target Subnet
- Your wallet has a balance of the Subnet Native Token(Specified under _alloc_ in your [Genesis File](/build/subnets/manage-subnets/upgrade/customize-subnet#genesis)).

Step 1: Setting up Core[​](#step-1-setting-up-core "Direct link to heading")
----------------------------------------------------------------------------

### **EVM Subnet Settings:** [(EVM Core Tutorial)](/build/subnets/deploy-a-subnet/fuji-testnet#connect-with-core)[​](#evm-subnet-settings-evm-core-tutorial "Direct link to heading")
...
You can use Subnet-EVM just like you use C-Chain and EVM tools. Only differences are `chainID` and RPC URL. For example you can deploy your contracts with [hardhat quick start guide](/build/dapps/smart-contract-dev/toolchains/hardhat) by changing `url` and `chainId` in the `hardhat.config.ts`.