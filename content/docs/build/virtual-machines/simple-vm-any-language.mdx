---
title: How to Build a Simple VM From Scratch
sidebarTitle: Simpe VM in Any Language
---
...
Minimal Implementation[​](#minimal-implementation "Direct link to heading")
---------------------------------------------------------------------------
...
Starting Process[​](#starting-process "Direct link to heading")
---------------------------------------------------------------
...
VM Initialization[​](#vm-initialization "Direct link to heading")
-----------------------------------------------------------------
...
### Pre-Initialization Sequence[​](#pre-initialization-sequence "Direct link to heading")
...
### Initialization Sequence[​](#initialization-sequence "Direct link to heading")
...
- [VM.SetState](https://buf.build/ava-labs/avalanche/docs/main:vm#vm.VM.SetState) _If you had specified `true` in the `StateSyncEnabled` result_
...
- [VM.GetOngoingSyncStateSummary](https://buf.build/ava-labs/avalanche/docs/main:vm#vm.VM.GetOngoingSyncStateSummary) _If you had specified `true` in the `StateSyncEnabled` result_
...
- [VM.SetState](https://buf.build/ava-labs/avalanche/docs/main:vm#vm.VM.SetState)
    ...
    - Return : a [SetStateResponse](https://buf.build/ava-labs/avalanche/docs/main:vm#vm.SetStateResponse) built from the genesis block.
- [VM.Connected](https://buf.build/ava-labs/avalanche/docs/main:vm#vm.VM.Connected) (for every other node validating this Subnet in the network)
...
#### Transaction Gossiping Sequence[​](#transaction-gossiping-sequence "Direct link to heading")
...
NodeX has received a new transaction on it's json-RPC endpoint : _on nodeX_

- [`AppSender.SendAppGossip`](https://buf.build/ava-labs/avalanche/docs/main:appsender#appsender.AppSender.SendAppGossip) (_client_)
...
#### Block Building Sequence[​](#block-building-sequence "Direct link to heading")

Whenever your VM is ready to build a new block, it will initiate the block building process by using the [Messenger](https://buf.build/ava-labs/avalanche/docs/main:messenger) service. Supposing that nodeY wants to build the block. _you probably will implement some kind of background worker checking every second if there are any pending transactions_ :

_on nodeY_

- _client_ [`Messenger.Notify`](https://buf.build/ava-labs/avalanche/docs/main:messenger#messenger.Messenger.Notify)
...
#### Managing Conflicts[​](#managing-conflicts "Direct link to heading")
...
### json-RPC[​](#json-rpc "Direct link to heading")

To enable your json-RPC endpoint, you must implement the HandleSimple method of the [`Http`](https://buf.build/ava-labs/avalanche/docs/main:http) interface.
...
- [`Http.HandleSimple`](https://buf.build/ava-labs/avalanche/docs/main:http#http.HTTP.HandleSimple)
    ...
    - Analyze, deserialize and handle the request
        ...
        _for example, if the request represents a transaction, we must deserialize it, check the signature, store it and gossip it to the other nodes using the [messenger client](#block-building-sequence))_
