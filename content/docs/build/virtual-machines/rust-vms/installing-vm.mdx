...
sidebarTitle: Installing Your VM
---

AvalancheGo searches for and registers VM plugins under the `plugins` [directory](/nodes/configure/configs--flags#--plugin-dir-string).
...
Now, this VM's static API can be accessed at endpoints `/ext/vm/timestampvm-rs` and `/ext/vm/timestamp-rs`. For more details about VM configs, see [here](/nodes/configure/configs--flags#virtual-machine-vm-configs).
