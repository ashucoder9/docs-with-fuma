...
<Callout title="Note">
Ethereum has its own notion of `networkID` and `chainID`. These have no relationship to Avalanche's view of networkID and chainID and are purely internal to the [C-Chain](/docs/learn/primary-network#c-chain). On Mainnet, the C-Chain uses `1` and `43114` for these values. On the Fuji Testnet, it uses `1` and `43113` for these values. `networkID` and `chainID` can also be obtained using the `net_version` and `eth_chainId` methods.
</Callout>
...
<Callout title="info">
On the [public API node](/docs/tooling/rpc-providers#supported-apis), it only supports C-Chain
...
On localhost, use `ws://`. When using the [Public API](/docs/tooling/rpc-providers.md) or another
...
<Callout title="info">
For batched requests on the [public API node](/docs/tooling/rpc-providers.md) , the maximum
...
Not recommended for use on Mainnet. See warning notice in [Keystore API](/docs/api-reference//avalanchego/keystore-api.md).
</Callout>

Export an asset from the C-Chain to X-Chain or P-Chain. If exporting to the X-Chain, you must call the
X-Chain's [`avm.import`](/docs/api-reference//avalanchego/x-chain/api.md#avmimport).
...
**DEPRECATED—instead use** [`avax.export`](/docs/api-reference//avalanchego/c-chain/api.md#avaxexport).

Send AVAX from the C-Chain to X-Chain or P-Chain. If exporting to the X-Chain, you must call the
X-Chain's [`avm.import`](/docs/api-reference//avalanchego/x-chain/api.md#avmimport) with assetID `AVAX`
...
**DEPRECATED—instead use** [`avax.import`](/docs/api-reference//avalanchego/c-chain/api.md#avaximport)
...
with the Admin API enabled, use [C-Chain config flag`--coreth-admin-api-enabled:true`](/docs/nodes/configure/chain-configs/C.md#coreth-admin-api-enabled).
